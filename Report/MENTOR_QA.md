...
**Q9. Vì sao 18 bảng mà không phải nhiều/ít hơn? Cơ sở nào?**
Em suy schema **từ 12 màn mockup**, không đoán trước: **mỗi bảng phải có ít nhất một màn "đòi"
nó ra**, bảng nào không chỉ được màn nào cần thì cắt. 18 bảng chia 6 nhóm A–F (danh tính/nền,
onboarding, campaign, content, tiền, cắt-ngang). Bản đầu AI sinh 45 bảng — em bỏ, vì không giải
thích nổi từng bảng khi mentor hỏi. Tài liệu `docs/DATA_MODEL.md` ghi rõ từng bảng: vì sao tồn
tại + khóa unique nào chống bug gì.

**Q10. Tiền lưu kiểu gì? Vì sao?**
**Minor units** (`*_minor BIGINT`) + cột `currency` đi kèm, **không dùng float**. VND exponent 0
(1đ = 1 minor), PHP exponent 2 (1 peso = 100 centavo). Float sai số làm lệch 1 xu × hàng vạn
giao dịch = mất niềm tin/mất tiền. Cột `net` **không lưu rời** — luôn tính lại `net = gross −
tax` để không có 2 nguồn sự thật. Ra JSON thì BigInt → Number (dưới 2^53 nên an toàn).

**Q11. Chỉ 1 khóa unique bất kỳ — nói nó chống bug gì?**
- `earning.submission_id` UNIQUE → **exactly-once earning** (1 content duyệt không sinh 2 thu nhập).
- `participation(profile_id, campaign_id)` UNIQUE → **join idempotent** (bấm 2 lần không giữ 2 suất).
- `payout_attempt.provider_ref` UNIQUE → **chống double-pay** (callback/retry chỉ ghi 1 lần).
- `ledger_entry(ref_type, ref_id, entry_type)` UNIQUE → 1 sự kiện không ghi sổ 2 lần.
- `users(auth_provider, provider_subject)` UNIQUE → 1 danh tính SSO không tạo 2 tài khoản.

**Q12. Snapshot điều khoản là gì và lưu ở đâu? (bài toán #5)**
Lúc Join, em **copy** đơn giá + trigger + pricing vào chính hàng `participation`
(`snapshot_reward_minor`, `snapshot_trigger_type`…). Nếu Admin đổi giá campaign sau đó, **earning
vẫn tính từ snapshot lúc join**, không bị hồi tố. Snapshot để **nullable** vì người đang
`WAITLISTED` chưa có suất nên chưa có điều khoản; khi được đôn lên mới ghi snapshot lúc đó.
...
**Q16. 🟢 Hai creator bấm Join suất cuối cùng lúc — xử lý sao cho không oversell? (bài toán #3)**
Join chạy trong **1 transaction, serial-hóa bằng khóa hàng campaign**: `SELECT campaign … FOR
UPDATE` → creator thứ 2 **xếp hàng chờ khóa**. Trong khóa mới đếm suất còn hiệu lực rồi ghi;
người sau vào khóa đọc lại = 0 → không giữ được suất. Cộng `UNIQUE(profile, campaign)` → bấm 2
lần cũng không thành 2. **Không bao giờ oversell dù 100 người bấm cùng lúc.** Em có **test tự
động RACE**: 3 creator bắn request đồng thời vào campaign 1 suất → đúng 1 JOINED, 2 WAITLISTED
(#1, #2), `slotsLeft = 0`. Code: `apps/api/src/campaign/join.service.ts`.

**Q17. FOR UPDATE có phải là "lựa chọn" không? Còn cách nào khác?**
Với ràng buộc "phân xử tại điểm commit, trong 1 DB", FOR UPDATE (pessimistic lock hàng campaign)
là **lời giải đúng, không phải khẩu vị**. Có 2 biến thể khác cùng họ: **optimistic** (cột
version + retry khi đụng) — hợp khi tranh chấp thấp; và **atomic conditional update**
(`UPDATE … SET slots_taken = slots_taken+1 WHERE slots_taken < slots_total` rồi kiểm rowcount).
Em chọn FOR UPDATE vì trong khóa còn phải làm nhiều bước (kiểm KYC, strike, snapshot, đôn
waitlist) — gom hết trong một vùng serial-hóa dễ suy luận đúng hơn là chuỗi conditional update.
...
**Q21. 🟡 Ops double-click "Approve" thì có tạo 2 thu nhập không? (bài toán #7)**
Không — nhờ `earning.submission_id` **UNIQUE** + transaction: earning gắn 1-1 với submission
được duyệt, cú approve thứ 2 đụng unique → không sinh bản thứ 2. (Phần này thuộc **N11**, đã thiết
kế khóa trong schema, sẽ code + test double-click ở Tuần C.)

**Q22. 🟡 Cổng thanh toán trả về UNKNOWN/timeout thì làm gì? (bài toán #4-payout)**
3 kết cục: **Success → PAID**; **Fail xác nhận → hoàn tiền về balance đúng 1 lần**; **UNKNOWN/
timeout → GIỮ tiền (reserve), chờ đối soát** — tuyệt đối **không release vội** vì nếu provider
thật ra đã chuyển tiền thì release = **double-pay**. `payout_attempt.provider_ref` UNIQUE để
callback/retry chỉ ghi một lần. (Thiết kế xong trong schema, code ở **N14–N15**.)

**Q23. 🟡 Sổ tiền ghi sai thì sửa thế nào? (bài toán #6)**
`ledger_entries` **append-only** — không UPDATE/DELETE bản ghi tiền. Sửa sai = ghi **bút toán
đảo** (`REVERSAL`) có `reversal_of_id` link về bản gốc → luôn tái dựng được lịch sử, audit được.
Đây là chuẩn kế toán, không phải lựa chọn tùy tiện.
...
Không. `assertStaffForCountry` kiểm `role_assignments` của phiên: Ops VN mở case PH bằng ID trực
tiếp → **404** (không phải 403, để không lộ sự tồn tại của tài nguyên nước khác). Mọi query
nghiệp vụ đều scope theo `country_id` của phiên, **không theo tham số client**. Đây là bài toán
khó #1, có test kiểm chứng (`Ops PH không review được case VN → 404`).
