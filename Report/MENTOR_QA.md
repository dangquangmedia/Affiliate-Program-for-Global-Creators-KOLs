...
> Giả lập buổi hỏi đáp sau khi trình bày prototype **Affiliate GLOBAL**. Trạng thái tại
> 22/07/2026: **money spine (N11-N15) đã xong**, và backend đã **viết lại toàn bộ sang Go**
> theo yêu cầu mentor (`apps/api-go`), giữ nguyên mọi quyết định/bất biến bên dưới — Nhóm 0-8
> vẫn đúng nguyên văn vì đó là quyết định sản phẩm/dữ liệu, không đổi khi đổi ngôn ngữ backend.
> Mỗi câu: **em đóng vai mentor đặt câu hỏi** → **câu trả lời ngắn gọn nhưng đủ ý**, có trỏ tới
> chỗ chứng minh trong code khi cần.
>
...
phức tạp gấp bội, dễ sai tiền. Đội 1 người: microservices chỉ thêm chi phí vận hành (deploy,
network, tracing) mà không đổi được giá trị nào ở quy mô này. Em vẫn **chia module ranh giới
rõ** (`apps/api-go/internal/{auth,country,kyc,campaign,content,earnings,reconciliation,
payout,audit}`) nên sau này thật sự cần thì tách được; ngược lại (gộp lại) mới khó. Quyết định
này giữ nguyên khi backend viết lại sang Go — đây là quyết định kiến trúc, không phụ thuộc ngôn
ngữ. → chi tiết: `docs/ARCHITECTURE.md`.

**Q14. Một request đi qua đâu? Server tin cái gì từ client?**
`Client --(Bearer token)--> chi middleware (RequestID/Recovery/AccessLog/CORS) --> handler -->
service --> pgx/sqlc --> Postgres`. **Nguyên tắc lòng tin: server không bao giờ tin danh
tính/vai/quốc gia từ client.** Danh tính + vai lấy từ **session trong DB**
(`auth.Service.ResolveSession` tra `sessions → users → role_assignments`), không từ body/header
tự khai. Route `/vn` `/ph` chỉ là **ý định hiển thị**; quyền thao tác nước nào do
...
Toàn bộ 36 nghiệp vụ chạy trên Go, xác nhận bằng nhiều lớp bằng chứng độc lập: **105/105 test
case cũ chạy lại qua HTTP thật** (`test:api:parity`, gồm cách ly cross-country, RBAC, join
idempotent, snapshot, exactly-once earning, payout 3 kết cục), **13/13 probe differential
Nest↔Go** (đã bắt và sửa 2 lệch hành vi thật khi port), **25/25 Playwright E2E trên Go**
(login→chọn nước→KYC→duyệt→join→content→earning→đối soát→payout), và **`go test -race`** cô lập
sạch cho 4 điểm race bắt buộc (join, content review, reconciliation lock, payout). Lint/vet/
typecheck sạch, `govulncheck` không lỗ hổng. Reclaim test dùng mẹo đẩy `submit_deadline_at` về
quá khứ để khỏi chờ 48h thật. → chi tiết: `Report/GO_WEEK6_COMPLETION.md`,
`docs/GO_API_PARITY_MATRIX.md`.

**Q28. Test RACE thật sự chạy song song hay giả lập tuần tự?**
Bắn nhiều request đồng thời (`Promise.all` phía harness / goroutine phía Go race test) vào cùng
endpoint. Nếu cơ chế sai thì sẽ oversell (≥2 JOINED) hoặc double-earning/double-lock. Kết quả ổn
định: join 1 JOINED + 2 WAITLISTED; content double-approve → 1 earning + 409 lần 2; reconciliation
double-lock → 409; payout 2 OTP → không reserve vượt balance — chứng minh claim có điều kiện
(`WHERE state=X`) và `SELECT ... FOR UPDATE` serial-hóa đúng, kể cả dưới `go test -race`.
...
## Nhóm 9 — Hướng phát triển tiếp

**Q32. Bước tiếp theo cụ thể là gì?**
Money spine (content→earning→ledger→đối soát→payout) và toàn bộ backend Go rewrite **đã xong**
(36/36 nghiệp vụ, parity + differential + E2E + race xanh — xem `Plan/GO_BACKEND_REWRITE_PLAN.md`).
Còn lại thuần **hạ tầng deploy**: Tuần 7 (Cloud Run/Cloud SQL/Secret Manager staging trên GCP) và
Tuần 8 (soak test + diễn tập rollback) — đang **tạm dừng**, chờ quyền truy cập GCP thật
(project id/billing/service account); không ảnh hưởng logic nghiệp vụ đã kiểm chứng.

**Q33. Nợ kỹ thuật / rủi ro còn lại?**
(1) Chưa deploy staging thật (GCP) — mới chạy local, xem Q32. (2) Không có rate-limit ở
login/OTP/payout — cần trước production thật. (3) Token session lưu `localStorage`, không phải
httpOnly cookie — thiếu một lớp phòng thủ sâu nếu có XSS. (4) OTP mock trả thẳng mã trong
response (đã công bố, bỏ khi nối OTP thật). (5) Go API chưa có security header
(CSP/HSTS/X-Frame-Options) — chuẩn hoá lúc lên Cloud Run. (6) Scheduler thu hồi hiện có thể tắt/
bật qua env, cần bật + giám sát khi production.

**Q34. Nếu chỉ được thêm 1 tuần nữa thì ưu tiên gì?**
Deploy staging thật trên GCP (Tuần 7) — vì logic nghiệp vụ (3 bài toán khó: exactly-once, payout
3 trạng thái, ledger đảo) đã chứng minh xong bằng test; giá trị còn thiếu lớn nhất cho một buổi
bảo vệ là **chạy được trên hạ tầng thật**, không chỉ trên máy local.
