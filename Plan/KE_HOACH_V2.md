...
> Nguồn yêu cầu: `Plan/docs/Book1.xlsx` (đề bài thực tập) + 5 bước & 4 tiêu chí mentor giao.
...
> nhiều hơn mức người làm hiểu sâu. Mục tiêu là mentor đánh giá cao **năng lực lõi**, không
> phải đếm số tính năng.

## 1. Đề bài tóm tắt (từ Book1.xlsx)
...
- Thang điểm: **0.4** must-flow chạy E2E · **0.25** DB/cách ly/code sạch · **0.15** UI/i18n/tiền tệ
  · **0.1** tài liệu/demo · **0.1** chủ động.
- Yêu cầu mentor (ảnh): 5 bước (Product → Database → Kiến trúc → Coding → Hạ tầng); không cần
...
## 2. Luồng tổng thể — 5 bước mentor ↔ 4 tuần
...
**Vì sao thứ tự này** (trả lời khi mentor hỏi "sao làm X trước Y"):
...
**Nguyên tắc cắt** (nói được với mentor): không bao giờ cắt cách ly country / tính đúng của
...
`load-env.ts`, pattern `@Inject()` tường minh, PrismaPg adapter, `Plan/docs/Book1.xlsx`.
